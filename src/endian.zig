...
pub fn store_u64_be(buf: []u8, x: u64) void {
    std.debug.assert(buf.len >= 8);
    buf[0] = @intCast((x >> 56) & 0xff);
    buf[1] = @intCast((x >> 48) & 0xff);
    buf[2] = @intCast((x >> 40) & 0xff);
    buf[3] = @intCast((x >> 32) & 0xff);
    buf[4] = @intCast((x >> 24) & 0xff);
    buf[5] = @intCast((x >> 16) & 0xff);
    buf[6] = @intCast((x >> 8) & 0xff);
    buf[7] = @intCast((x >> 0) & 0xff);
}
pub fn store_u64_le(buf: []u8, x: u64) void {
    std.debug.assert(buf.len >= 8);
    buf[0] = @intCast((x >> 0) & 0xff);
    buf[1] = @intCast((x >> 8) & 0xff);
    buf[2] = @intCast((x >> 16) & 0xff);
    buf[3] = @intCast((x >> 24) & 0xff);
    buf[4] = @intCast((x >> 32) & 0xff);
    buf[5] = @intCast((x >> 40) & 0xff);
    buf[6] = @intCast((x >> 48) & 0xff);
    buf[7] = @intCast((x >> 56) & 0xff);
}
...
test "store_u32_le edge cases" {
    var b0 = [_]u8 { 0x00, 0x00, 0x00, 0x00 };
    store_u32_le(&b0, 0x00000000);
    try std.testing.expectEqual(
        [_]u8 { 0x00, 0x00, 0x00, 0x00 },
        b0,
    );
    ...
    var b4 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u32_le(&b4, 0x12345678);
    try std.testing.expectEqual(
        [_]u8 { 0x78, 0x56, 0x34, 0x12, 0x00 },
        b4,
    );
}

test "store_u64_be basic" {
    var b = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_be(&b, 0x1234567890abcdef);
    try std.testing.expectEqual(
        [_]u8 { 0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef },
        b,
    );
}

test "store_u64_be edge cases" {
    var b0 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_be(&b0, 0x0000000000000000);
    try std.testing.expectEqual(
        [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
        b0,
    );
    
    var b1 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_be(&b1, 0xffffffffffffffff);
    try std.testing.expectEqual(
        [_]u8 { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
        b1,
    );

    var b2 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_be(&b2, 0xaa55aa55aa55aa55);
    try std.testing.expectEqual(
        [_]u8 { 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55 },
        b2,
    );
    
    var b3 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_be(&b3, 0x55aa55aa55aa55aa);
    try std.testing.expectEqual(
        [_]u8 { 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa },
        b3,
    );
    
    var b4 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_be(&b4, 0x1234567890abcdef);
    try std.testing.expectEqual(
        [_]u8 { 0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef, 0x00 },
        b4,
    );
}

test "store_u64_le basic" {
    var b = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_le(&b, 0x1234567890abcdef);
    try std.testing.expectEqual(
        [_]u8 { 0xef, 0xcd, 0xab, 0x90, 0x78, 0x56, 0x34, 0x12 },
        b,
    );
}

test "store_u64_le edge cases" {
    var b0 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_le(&b0, 0x0000000000000000);
    try std.testing.expectEqual(
        [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
        b0,
    );
    ...
    var b1 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_le(&b1, 0xffffffffffffffff);
    try std.testing.expectEqual(
        [_]u8 { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
        b1,
    );
    ...
    var b2 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_le(&b2, 0xaa55aa55aa55aa55);
    try std.testing.expectEqual(
        [_]u8 { 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa },
        b2,
    );
    ...
    var b3 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_le(&b3, 0x55aa55aa55aa55aa);
    try std.testing.expectEqual(
        [_]u8 { 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55 },
        b3,
    );
    ...
    var b4 = [_]u8 { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    store_u64_le(&b4, 0x1234567890abcdef);
    try std.testing.expectEqual(
        [_]u8 { 0xef, 0xcd, 0xab, 0x90, 0x78, 0x56, 0x34, 0x12, 0x00 },
        b4,
    );
}
